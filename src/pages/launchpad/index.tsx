    ...
    <div className="px-4 md:px-10 xl:px-0">
      <div className=" text-white py-24 md:py-32 border-b border-[#1F1F1F] ">
        <div className=" max-w-7xl mx-auto ">
          <div className="flex justify-start flex-row mb-4 text-2xl">Upcoming IDOs</div>
            ...
            <div className="flex flex-col	 xl:flex-row justify-between bg-W3GBG p-4 py-10  md:p-10    bg-cover	relative">
              <div className="xl:w-7/12 z-10">
                <div className="flex mb-3 xl:mb-6 ">
                  ...
                  <div>
                    <div className="text-2xl">{show_project.project_name}</div>
                    <div className="font-light">$ {show_project.project_token_name}</div>
                    <div className="text-xs text-gray-400 line-clamp-3 ">{show_project.project_h1}</div>
                  </div>
                </div>
                <div className="z-10 block xl:hidden">
                  <div className=" flex  mb-3 xl:mb-6">Coming Soon:</div>
                ...
                <div className="text-xs md:text-sm text-center xl:text-base flex justify-between mb-2">
                  <div>Initial valuation: {show_project.project_valuation}k</div>
                  <div>Initial token price: ${show_project.project_token_price}</div>
                  <div>Base allocation: ${show_project.project_allocation}</div>
                </div>
                <div className="flex mt-4  bg-[#222A35] h-6 rounded-full items-center justify-start">
                  <div className="bg-[#327FCC] h-6 rounded-full pl-5" style={{ width: `20%` }}>
                    20%
                  </div>
                </div>
              ...
              <div className="z-10 hidden xl:block">
                <div className=" flex justify-end mb-6">Coming Soon:</div>
          ...
          <div className="mt-12 ">
            ...
            <div className="hidden xl:grid md:grid-cols-3 xl:grid-cols-3  gap-8 mt-5">
              {project.map(item => (
                <Link passHref key={item.project_name} href={`/launchpad/details/${show_project.project_name}`}>
                  <div className="rounded-lg cursor-pointer">
                    ...
                    <div className="rounded-b-lg p-5 relative bg-W3GNavigationBorder">
                      ...
                      <div className="mt-5">
                        ...
                        <div className="text-xs text-[#A3A3A3] mt-1">$ {item.project_token_name}</div>
                        <div className="flex justify-between mt-3 text-sm">
                          <div className="text-[#A3A3A3]">Total Raise</div>
                          <div className="">$ {item.project_token_total_raise}</div>
                        </div>
                        <div className="flex justify-between mt-1 text-sm">
                          <div className="text-[#A3A3A3]">Porsonal Allcation</div>
                          <div>$ {item.project_porsonal_allcation} Max</div>
                        </div>
                      </div>
                    </div>
                    <div></div>
          ...
          <div className="mt-6 xl:mt-12">
            <div className="flex justify-between">
              <div>
                <div className="text-2xl">Funded Projects</div>
                <div className="text-white text-xs text-slate-500">History successfully project</div>
              </div>
