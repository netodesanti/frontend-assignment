...
const Hot = () => {
  ...
  return (
    <>
      <div className="text-2xl text-white font-semibold mb-10">🔥 Hot Collections</div>
      <div className="flex overflow-auto xl:overflow-hidden py-2 px-4  grid-cols-3 gap-9 xl:grid">
        {hot.map(item => (
          <div
            key={item.name}
            className="flex w-72 mr-36  rounded-xl p-2  transform  hover:scale-105 transition duration-300"
          >
            <img className="md:w-40 md: h-40 rounded-lg" src={item.img} alt="" />
            <div className="ml-4   ">
              <div className="flex mb-4 w-32 ">
                <div className="text-gray-100 text-xl font-semibold  truncate">{item.name}</div>
              ...
              <div className="text-gray-400 text-base mb-3 font-semibold">Floor Price</div>
              <div className="flex">
                <img className="w-6 rounded-full border border-gray-500" src="/img.png" alt="" />
                <div className="text-red-400 ml-2">{item.token}</div>
              </div>
            </div>
          </div>
        ))}
              ...
              <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-12 my-10">
                  {collections.map(item => (
                    <Link passHref key={item.name} href="/marketplaces/asset">
                      <div className="transform scale-100 hover:shadow hover:scale-105 transition duration-300 cursor-pointer">
                        ...
                        <div className="mt-4">
                          <div className="text-base font-semibold text-gray-200 truncate">{item.name}</div>
                          <div className="flex ">
                            <div className="text-gray-400 text-sm mr-1">Owned by</div>
                            <div className="text-red-500 font-light text-sm">{item.address}</div>
                          </div>
                          <div className="flex mt-4 justify-between">
                            <div>
                              <div className="text-gray-500 text-sm">Sell Price</div>
                              <div className="flex">
                                <img className="w-6 border border-gray-700 rounded-full" src="/img.png" alt="" />
                                <div className="ml-1 text-gray-100">{item.sell}</div>
                              </div>
                            </div>
                            <div>
                              <div className="text-gray-500 text-sm">Best Offer</div>
                              <div className="flex">
                                <img className="w-6 border border-gray-700 rounded-full" src="/img.png" alt="" />
                                <div className="ml-1 text-gray-100">{item.offer}</div>
                              </div>
                            </div>
                            <div>
                              <div className="text-gray-500 text-sm">Last Price</div>
                              <div className="flex">
                                <img className="w-6 border border-gray-700 rounded-full" src="/img.png" alt="" />
                                <div className="ml-1 text-gray-100">{item.last}</div>
                              </div>
                            </div>
                          </div>
                        </div>
