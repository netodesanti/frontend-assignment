          ...
          {featured.map(item => (
            <Link passHref key={item.name} href="/marketplaces">
              ...
              <div className="md:mx-5 my-3 w-72 md:w-96  xl:w-72 mx-5">
                <div className="text-gray-100  truncate text-2xl font-semibold mb-5">{item.name}</div>
                <div className="flex mb-5">
                  <img className="w-6 rounded-full" src={item.logo} alt="" />
                  <div className="text-gray-300 text-xs ml-2 mt-1">{item.address}</div>
                </div>
                <div className="text-gray-300 text-sm mb-5">Floor Price</div>
                <div className="flex mb-5">
                  <img className="w-8 rounded-full border border-gray-400" src="/img.png" alt="" />
                  <div className="ml-3 text-red-400 text-xl font-semibold">{item.data}</div>
                </div>
                <div className="text-gray-200 h-20 pt-2 md:h-20 overflow-hidden">{item.h1}</div>
              </div>
              {/* </a> */}
            </Link>
          ))}
    ...
    <>
      <div className="mt-20">
        <div className="flex justify-between ">
          <div className="text-white text-3xl">NFT Ranking list</div>
        ...
        <div className="flex justify-between mt-10 overflow-auto text-white">
          {ranking.map(item => (
            <div key={item.title} className="mt-10 mx-10">
              <div className="mx-6">{item.title}</div>
              {item.info.map(item => (
                <Link passHref href="/marketplaces" key={item.name}>
                  {/* <a  className="flex mt-6 hover:bg-gray-800  py-2 px-6 pr-16 "> */}
                  <div className="text-2xl font-semibold mt-2 mr-8">{item.rank}</div>
                  <div className="flex">
                    <img className="w-14 rounded-lg border border-gray-400 mr-4" src={item.img} alt="" />
                    <div>
                      <div className="mb-2 font-semibold truncate w-32">{item.name}</div>
                      <div className="flex">
                        <img
                          className={item.token ? 'w-6 border mr-2  border-gray-400 rounded-full' : 'hidden'}
                          src={item.token}
                          alt=""
                        />
                        <div className={classNames([item.textstyle], 'text-gray-400 text-sm mt-0.5')}>{item.data}</div>
                      </div>
                    </div>
                  </div>
                  {/* </a> */}
...
const Trending = () => {
  return (
    <>
      <div className="mt-20">
        <div className="text-white text-3xl ">Trending in all categories</div>
              ...
              <div className="">
                ...
                <div className="mt-2">
                  ...
                  <div className="flex justify-between mt-2">
                    ...
                    <div className="text-gray-100">{item.items}</div>
                  </div>
                </div>
              </div>
              {/* </a> */}
