...
const sort = [{ type: 'Price:High to Low' }, { type: 'Price:Low to High' }];
const ListItems = () => {
    ...
    <>
      ...
      <div className="md:flex justify-between mt-3">
        <div className="flex text-sm md:text-lg ">
          <div className="hidden md:inline-block mt-1 mr-8 text-gray-300">Status</div>
          <Tab.Group>
            <Tab.List className="">
              <div>
                {status.map(item => (
                  <Tab
                    ...
                    className={({ selected }) =>
                      classNames(
                        ...
                        selected ? 'bg-red-500' : ' hover:bg-white/[0.12] hover:text-white',
                      )
                    }
                  >
                    {item.type}
        ...
        <div className=" grid grid-cols-1 md:grid-cols-3 xl:grid-cols-5 gap-8">
          {items.map(item => (
            <Link passHref href="/#" key={item.img}>
              ...
              <div className="mt-3 ">
                <div className="text-white font-semibold text-sm w-48 truncate">{item.name}</div>
                <div className="text-gray-400 text-sm">Parcel {item.parcel}</div>
              </div>
              <div className="flex mt-4 text-white justify-between text-sm text-center">
                ...
                <div>
                  <div className="text-gray-400">Best Offer</div>
                  <div>{item.bestOffer}</div>
                </div>
                <div>
  ...
  return (
    <>
      <div className="flex text-sm md:text-base md:mt-5">
        <div className="hidden md:inline-block  mt-1 mr-8 text-gray-300">Event Type</div>
        <Tab.Group>
          <Tab.List className="">
            <div>
              {type.map(item => (
      ...
      <div className="shadow overflow-auto border-b my-10  border-gray-600 sm:rounded-lg">
        <table className="min-w-full border-t border-gray-600 divide-y  divide-gray-600">
          <thead className="bg-gray-900 bg-opacity-60 ">
            <tr>
              {table.map(title => (
                <th key={title.title} scope="col" className="px-6 py-3 text-left text-sm font-semibold text-gray-200  ">
                  {title.title}
                </th>
              ))}
            </tr>
            ...
            {tableDetail.map(item => (
              <Link passHref key={item.id} href="/">
                <tr className="hover:bg-gray-800">
                  <td className="px-6 py-6 whitespace-nowrap text-sm font-semibold text-white">{item.type}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-white">
                    <div className="flex">
                      <img className="w-7 rounded-full mr-1" src={item.img} alt="" />
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-base text-white">{item.quantity}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-white">{item.from}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-white">{item.to}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-base text-white">{item.time}</td>
                </tr>
              </Link>
            ))}
          </tbody>
              ...
              <Tab
                ...
                className={({ selected }) =>
                  classNames(
                    ...
                    selected ? ' text-yellow-50 border-b shadow' : ' hover:bg-white/[0.12] hover:text-white',
                  )
                }
              >
                {category}
        ...
        <div className=" mx-auto  ">
          <div className="relative sm:overflow-hidden">
            <div className="absolute inset-0">
              <img className="h-full w-full object-cover" src={message.img} alt="People working on laptops" />
            </div>
            <div className="relative px-4 py-16 sm:px-6 sm:py-24  mx-auto max-w-7xl"></div>
          </div>
            ...
            <div className="max-w-7xl relative px-5 py-16  sm:px-6  mx-auto ">
              ...
              <div className="xl:flex mt-10">
                <div className="xl:w-8/12">
                  ...
                  <div className="mt-5 text-gray-300 h-24 overflow-hidden">{message.h1}</div>
                  ...
                  <div>
                    <div className="flex">
                      <div className="w-1/2 text-center  border-r border-b   border-gray-400 py-5">
                        <div className="mb-2 text-gray-300 ">Items</div>
                        <div className="text-white font-semibold text-2xl">{message.items}</div>
                      </div>
                      <div className="w-1/2 text-center border-b border-gray-300 py-5 ">
                        <div className="mb-2 text-gray-300 ">Owners</div>
                        <div className="text-white font-semibold text-2xl">{message.owners}</div>
                      </div>
                    </div>
                    <div className="flex">
                      <div className="w-1/2 text-center border-r border-gray-400  py-5">
                        <div className="mb-2 text-gray-300 ">Avg Closed Price (24h)</div>
                        <div className="text-white font-semibold text-2xl">{message.price}</div>
                      </div>
                      <div className="w-1/2 text-center py-5 ">
                        <div className="mb-2 text-gray-300 ">Volume Traded</div>
                        <div className="text-white font-semibold text-2xl flex justify-center">
                          <img className="w-8 rounded-full border border-gray-500 mr-1" src="/img.png" alt="" />{' '}
                          {message.traded}
                        </div>
                      </div>
                    </div>
